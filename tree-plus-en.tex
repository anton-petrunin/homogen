...
An equilateral all-set-homogeneous metric space must be finite.
\end{thm}
...
Further, given a set $Q\subset M$, let
\[\vv pQ\ww=\bigcap_{q\in Q} \vv pq\ww.\]

\begin{thm}{Lemma}\label{lem:split-time}
Let $M$ be an all-set-homogeneous geodesic space.
For any point $p\in M$ and any nonempty set $Q\subset M$ there is a point $x\in \vv pQ\ww$ that maximizes the distance $|p-x|$.

\end{thm}

\parit{Proof.}
Let $r=\sup\set{|p-x|}{x\in \vv pQ\ww}$.

Choose a sequence $x_1,x_2,\ldots\in \vv pQ\ww$ such that the sequence $r_n=|p-x_n|$ is increasing  and $r_n\to r$ as $n\to\infty$.
Applying induction, we can assume
\[[px_1]\subset[px_2]\subset\dots \tag{${*}$}\]
Indeed, suppose $[px_1]\subset\ldots\subset[px_{n-1}]$.
Choose a geodesic $[px_n]$ and points $x'_1,\ldots,x_{n-1}\in [px_n]$ such that $|p-x'_i|=|p-x_i|$.
Since $M$ is all-set-homogeneous there is an isometry of $M$ that fixes $\{p\}\cup Q$ and moves $x'_i$ to $x_i$;
this isometry moves $x_n$ to a new position such that $[px_{n-1}]\subset [px_n]$.

The nested union of geodesics in $({*})$ is a geodesic.
By \ref{obs:geodesically-complete} it extends to the limit point $x$ of the sequence, hence the result.
\qeds

\subsection*{Splitting function}

Consider a point $p$ and a two-point set $Q=\{q_1,q_2\}$ in an all-set-homogeneous geodesic space~$M$.
...
By maximality of the splitting point, we have that $\sigma_1(t)=\sigma_2(t)$ and $s(t)=0$ if $t\le r_2-r_0$ --- a contradiction.
Here we apply \ref{lem:split-time} to two pairs:
$\sigma_0(r_2)$, $\{\sigma_1(r_0),\sigma_2(r_0)\}$
and $\sigma_0(r_0)$, $\{\sigma_1(r_2),\sigma_2(r_2)\}$.
...
If $s(r)=s'(r')$, then $\sigma'_1(r')\mapsto\sigma_1(r)$, $\sigma'_2(r')\mapsto\sigma_2(r)$, and
...
The configuration of points provided by the lemma will be called \emph{$r$-rhombus}.

\parit{Proof.}
Suppose $0<R<\diam M$.

Since the geodesics are not unique, we may find a configuration of distinct points $v_1,v_2,q_1,q_2$ such that $|v_1-v_2|$ is small, $q_1,q_2\in \vv v_1v_2\ww$ and $|v_1-q_1|\z=|v_1-q_2|$ (and hence $|v_2-q_1|=|v_2-q_2|$).
...
Set $Q=\{q_1,q_2\}$,
and let $p_i$ be the point provided by \ref{lem:split-time} for $w_i$ and $Q$.
...
\vskip-0mm
\end{figure}
All marked geodesic segments in the picture have length $r$; black points are equidistant.
...
it is uniquely defined if $r<|p-x|<R$.
...
and $s(\eps)>2\cdot \delta$ $\Rightarrow$ $x_r=y_r$.
...
in other words, $M$ does not contain an embedded circle.
...
Then there is an infinite equilateral set $A\z\subset M$;
