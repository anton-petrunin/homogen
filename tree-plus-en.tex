...
Choose a geodesic $[px_n]$ and points $x'_1,\ldots,x'_{n-1}\in [px_n]$ such that $|p-x'_i|=|p-x_i|$ for each $i<n$.
Since $M$ is all-set-homogeneous there is an isometry of $M$ that fixes points $p$, $q_1$, $q_2$ and moves $x'_i$ to $x_i$;
...
Consider three points $p$, $q_1$, and $q_2$ in an all-set-homogeneous geodesic space~$M$.
...
The proof explains the following picture; here marked geodesic segments have length $r$, ands black points are at the distant $s(r)$ from each other.
\begin{figure}[ht!]
\centering\vskip-0mm
